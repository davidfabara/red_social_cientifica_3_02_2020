...
class usuarios{
	...
	{
		$con = conexion("root", ""); //llama a la funcion conexion de funciones.php ojo esta en espanol el nombre de esta funcion)
	...
	function verificar($usuario) //Para verificar si usuario existe
	{
		$con = conexion("root", ""); /* Se llama a la funcion "conexion" de funciones.php enviando argumentos usuario y contrasena para acceso a base de datos*/
...
class post{
	...
	function agregar($CodUsua, $titulo, $autor, $fecha, $categoria, $contenido, $img, $url)
	{
		$con = conexion("root", "");
	...
	function mostrarTodo_busqueda($buscar){
		$con = conexion("root", "");
...
class denuncias{
	...
	function mostrar($CodPost)
	{
	 ...
	 function verificar_denuncia($CodPost, $CodUsua){
