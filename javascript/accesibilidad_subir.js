...
function ejecutar_ayuda() {
    ...
    artyom.say( "Estas en la pagina principal, con comandos de voz pronuncia opción 1, para ir a la página principal, opcion 2, para buscar, opcion 3, para publicar, opcion 4, para ver solicitudes, opcion 5, para ver las noticias, opcion 6 para acceder a tu perfil, opcion 7 para cerrar , opcion 8 para subir una publicación, opcion 9 para acceder a la lista de publicaciones, disponible también comando ayuda para subir y comando ayuda para texto, adicional ayuda para sistemas híbridos");          

}

function ejecutar_ayuda_subir(){

    artyom.say('Los datos a introducir son: titulo, autor, fecha,categoria, resumen, introducción, contenido, conclusiones, referencias, subir archivo y demás detalles al final se envia todo pronunciando, enviar publicación. Sección para subir publicación, para el caso de categoria,resumen,introduccion,contenido,conclusiones,referencias pronuncia. Ejm resumen elemento número (seguido del número de elemento), seguido del tipo de input luego con: seguido de la información a suministrar, un ejemplo básico para crear el primer elemento de resumen sería: elemento número 1 de resumen seguido del dictado de texto, para crear una cita pronuncia, crear cita para seguido del imputa ya sea este resumen, introducción, contenido, conclusiones. Para referenicas, pronuncia, crear referencia');
}
function ejecutar_ayuda_texto(){

    artyom.say("Para crear un texto, pronunciar crear texto seguido del texto a procesar, para deletrear una frase, pronunciar deletrear seguido del caracter a procesar, para pegar la frase de deletrear con el texto, pronuncia pegar deletrear en texto, para el caso de pegar un texto en algún imput, pronunciar pegar seguido de la palabra texto o deletrear seguido de la palabra elemento seguido de la palabra número   seguido del número de elemento seguido del nombre del imput, ejm para pegar el texto creado en el primer input de resumen, sería, pegar texto en elemento número 1 de resumen, nota: también se puede aplicar este proceso para los comentarios.");
}
function ejecutar_ayuda_sistema_hibrido(){
    artyom.say("Para utilizar el teclado braile para suministrar texto y para enfocar elementos con la voz, pronuncia ejemplo, enfocar elemento 1 de resumen, esto posicionará el cursor en el primer elemento de la categoría de resumen, esto aplica para cualquier input concreto");
}
...
if (annyang) {
    ...
    var commands = {
        ...
        'leer comentarios de publicación *value': (value) => {
            ...
            if(document.body.contains( document.getElementsByName('comentarios-acumulados')[value].value)){
                artyom.say(document.getElementsByName('comentarios-acumulados')[value].value);
                console.log(artyom.say(document.getElementsByName('comentarios-acumulados')[value].value));
            }else{
        ...
        'deletrear *value': (value) => {
            ...
            if(deletreo == "")
                artyom.say("Decir palabras que evoquen el primer caracter para aumentar la presición, ejemplo david para el caracter d");
